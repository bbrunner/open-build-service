...
class Patchinfo < ActiveXML::Node
  ...
  def patchinfo_axml(project)
    ...
    xml.add_element('rating').text ='moderate'
    xml
  end


  #def create_patchinfo_from_request(project, req, pkg_name, diff, issues)
  def create_patchinfo_from_request(project, req, pkg_name, diff, issueList)
    project.check_write_access!
    @prj = project
    ...
    issueList.each do |issue|
      ...
      if issue.attributes['tracker'] == "cve"
        category = "security"
        issue.attributes['name'] = "CVE-" + issue.attributes['name']
      end
      e.set_attribute 'tracker', issue.attributes['tracker']
      e.set_attribute 'id', issue.attributes['name']
      cnt += 1
    end
   ...
   # end
    ...
    xml.add_element('category').text = category
    xml.add_element('summary').text = category.titleize + " update for " + pkg_name
